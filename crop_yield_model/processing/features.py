...
class YearOneHotEncoder(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        encoded_year = self.encoder.transform(X[[self.variable]])
        # Append encoded year features to X
        X[self.encoded_features_names] = encoded_year

        # drop 'year' column after encoding
