...
from crop_yield_model.config.core import DATASET_DIR, TRAINED_MODEL_DIR, config
...
def pre_pipeline_preparation(*, data_frame: pd.DataFrame) -> pd.DataFrame:
    """
    Prepares the data for the pipeline.
    """
    # Drop unnecessary fields
    for field in config.crop_config.unused_fields:
    ...
    # data_frame['hg/ha_yield'] = data_frame['hg/ha_yield'].astype(float)
    data_frame['Area'] = data_frame['Area'].astype(str)
    data_frame['Item'] = data_frame['Item'].astype(str)
    data_frame['Year'] = data_frame['Year'].astype(int)
    data_frame['average_rain_fall_mm_per_year'] = data_frame['average_rain_fall_mm_per_year'].astype(float)
    data_frame['pesticides_tonnes'] = data_frame['pesticides_tonnes'].astype(float)
    data_frame['avg_temp'] = data_frame['avg_temp'].astype(float)
    return data_frame
